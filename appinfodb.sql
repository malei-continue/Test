...
SELECT * FROM app_category WHERE parentId = NULL
...
UPDATE app_category SET parentId = 0 WHERE id  IN(1,2)
...
CREATE TABLE `app_info` (
  ...
  `modifyDate` DATETIME DEFAULT NULL COMMENT '最新更新时间',
  `categoryLevel1` BIGINT(30) DEFAULT NULL COMMENT '所属一级分类（来源于：app_category）',
  `categoryLevel2` BIGINT(30) DEFAULT NULL COMMENT '所属二级分类（来源于：app_category）',
...
SELECT d.`valueName`,ad.`valueName` FROM app_info a INNER JOIN data_dictionary d ON a.`status` = d.`valueId`
						    INNER JOIN data_dictionary ad ON a.`flatformId` = ad.`valueId`
	                                             WHERE d.`typeName`="APP状态" AND ad.`typeName`="所属平台" 
...
SELECT *,
(SELECT  valueName FROM data_dictionary dd WHERE dd.`typeName`="APP状态" AND dd.`valueId`=a.`status` ) statusName,
(SELECT  valueName FROM data_dictionary dc WHERE dc.`typeName`="所属平台" AND dc.`valueId`=a.`flatformId` ) flatformName ,
(SELECT categoryName FROM app_category av WHERE av.`id` = a.`categoryLevel1`) categoryName1,
(SELECT categoryName FROM app_category av WHERE av.`id` = a.`categoryLevel2`) categoryName2,
(SELECT categoryName FROM app_category av WHERE av.`id` = a.`categoryLevel3`) categoryName3,
(SELECT versionNo FROM app_version  ave  WHERE ave.`appId` = 53 ORDER BY  ave.createionDate  ) versionNo
FROM app_info  a LIMIT 0,5
